...
<body>
    ...
    <div class="client">
        <h3>Nom du client : <i>{{ $fac->client->nom }}</i> </h3>
        <h3>Telephone : <i>{{ $fac->client->telephone }} </i></h3>
    </div>
    <div class="fac">
        <h3> Num facture:<i> V0{{$fac->code_fac}}</i></h3>
     <h3>Date: <i>{{ $fac->created_at->format('d-m-Y H:m') }}</i> </h3>
    </div>
        ...
        <tbody>
            ...
            @foreach ($ventes as $vente)
                <tr>
                    ...
                    <td>{{ $vente->produit->nom . '/' . $vente->produit->description }}</td>
                    <td>{{ $vente->quandite }}</td>
                    <td>{{ $vente->prixu }}</td>
                    <td>{{ $vente->total }}</td>
                </tr>
            @endforeach
            <tr>
                ...
                <td>{{ $ventes->sum('total') }}</td>
            </tr>
        </tbody>
    </table>
    <p>Les marchandises ci-dessus sont vendues en bon etat: elles ne sont ni reprises ni echangees 30
        jours de garantie et pas de remboursement.</p>
    <h3 style="text-align:left">Signature du client</h3>
</body>
